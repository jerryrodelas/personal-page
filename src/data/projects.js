...
export const projects = [
...

  {
    ...
    description:
      "Built a mobile app that finds cheaper electricity, gas, and petrol/diesel options by combining a Go backend, a Python/FastAPI extraction microservice, and a React Native/Expo iOS app. For electricity and gas, Claude Haiku extracts retailer, tariff, usage, and billing period straight from a photographed or PDF bill — no OCR pipeline, no manual entry — and the app pulls live plan data from the Australian Energy Regulator's Consumer Data Right API to project an annual cost from what was actually charged and rank every comparable plan by savings. For fuel, it pulls live petrol/diesel/LPG pricing from NSW's FuelCheck API, searchable by postcode, suburb, or GPS, ranked cheapest-first with distance and station details. Solved two gnarly matching problems along the way: annualizing cost from what the bill actually charged rather than reconstructing a rate (which breaks on gas's tiered/inclining-block pricing), and fuzzy-matching a bill's printed retailer network name (\"Jemena Gas\") against the government's canonical network name (\"Jemena Coastal Network\"). Unified AER's unauthenticated CDR access and FuelCheck's OAuth2 client-credentials-plus-replay-protection auth behind one internal PlanProvider-style abstraction, so the comparison engine doesn't know or care which government API it's talking to. Added per-client and global rate limiting plus Azure budget alerts to guard against a leaked URL running up metered LLM and government API calls. Deployed on Azure Container Apps via Docker.",
    category: "mobile",
    status: "completed",
    tags: ["Go", "Python", "FastAPI", "Claude API", "React Native", "Expo", "Azure Container Apps", "Docker"],
    appStoreLink: "",
    playStoreLink: "",
    githubLink: "",
    liveLink: "",
